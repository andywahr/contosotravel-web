...
resource "azurerm_key_vault_secret" "serviceFQDN" {
  count        = "${var.backend == "kubernetes" ? 1 : 0}"
  name         = "ContosoTravel--ServiceFQDN"
  value        = "http://contosotravel-service.${element(concat(azurerm_kubernetes_cluster.aks.*.addon_profile.0.http_application_routing.0.http_application_routing_zone_name, list("")), 0)}/api/PurchaseItinerary"
  key_vault_id = "${var.keyVaultId}"
}
